...
if (isset($_POST['username'])) { 
    if (!empty($_POST['password']) && !empty($_POST['username'])) {
        $user = $pdo->read('access', ['username'=>$_POST['username'], 'password'=>$_POST['password']]);
    
        
        if (!empty($user)) {
            $_SESSION['guardmark_user_id'] = $user[0]['id'];

            $_SESSION['guardmark_username'] = $user[0]['username'];
            $_SESSION['guardmark_email'] = $user[0]['username'];

            $_SESSION['guardmark_role_id'] = $user[0]['role_id'];
            $_SESSION['guardmark_email'] = $user[0]['email'];
            $_SESSION['guardmark_user_id'] = $user[0]['user_id'];
...
if (isset($_POST['email'])) { 
    if (!empty($_POST['password']) &&  !empty($_POST['email'])) {
        if ($pdo->validateInput($_POST['email'], 'email')) {
            ...
            $user = $pdo->read('access', ['email'=>$_POST['email'], 'password'=>$_POST['password']]);
                        ...
                        <form method="post">
                            ...
                            <div class="ad-auth-form">
                                ...
                                <div class="ad-auth-feilds mb-30">
                                    <input type="email" placeholder="Email" name="email" id="email" class="ad-input" />
                                    <div class="ad-auth-icon">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 483.3 483.3">
                                            <path
                                                d="M424.3,57.75H59.1c-32.6,0-59.1,26.5-59.1,59.1v249.6c0,32.6,26.5,59.1,59.1,59.1h365.1c32.6,0,59.1-26.5,59.1-59.1    v-249.5C483.4,84.35,456.9,57.75,424.3,57.75z M456.4,366.45c0,17.7-14.4,32.1-32.1,32.1H59.1c-17.7,0-32.1-14.4-32.1-32.1v-249.5    c0-17.7,14.4-32.1,32.1-32.1h365.1c17.7,0,32.1,14.4,32.1,32.1v249.5H456.4z"
