...
public function createMulti($table, $data, $fileInputName = null, $imageColumns = null, $imageExtensionColumns = null)
{
    ...
    public function validateInput($input, $type)
    {
        ...
        if ($type === 'username') {
            ...
            // BRPN-specific checks
            if ($length !== 11 || !ctype_digit($input)) {
                $this->validationErr[] = "BRPN must be 11 digits long.";
                return false;
            }
    
            // Check BRPN algorithm
            $sum = 0;
            for ($i = 0; $i < 9; $i++) {
                $sum += $input[$i] * (10 - $i);
            }
            ...
            if ($input[9] != $verification_digit) {
                $this->validationErr[] = "Invalid BRPN. Check digit is incorrect.";
                return false;
            }
    
            // If all checks pass, the BRPN is valid
            return true;
        }
        elseif ($type === 'cnic') {
