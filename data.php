...
if ($_POST['__FILE__'] == "duitesSiteName") {
    ...
    if (!empty($_POST['site'])) {
        ...
        $guards = $pdo->customQuery("SELECT * FROM registration
            ...
            WHERE registration.status != 'Expired' AND registration.status = 'Verified' AND NOT EXISTS(SELECT * FROM duties WHERE registration.id = duties.id AND duties.status != 'Complete') AND availability LIKE '%".date("l", strtotime($site[0]['date']))."%'");
        ...
        if (isset($_POST['county_search']) && $_POST['county_search'] == "checked") {
            ...
            WHERE registration.status != 'Expired' AND registration.status = 'Verified' AND NOT EXISTS(SELECT * FROM duties WHERE registration.id = duties.id AND duties.status != 'Complete') AND state = '{$site[0]['state']}' AND availability LIKE '%".date("l", strtotime($site[0]['date']))."%'");
